...
# ============================================================
# Process one candidate in parallel
# ============================================================
...
def process_single_candidate(
    ...
    *,
    ...
    publisher_schema,
    publisher_backend_name: str,
    publisher_external_writes_enabled: bool,
    existing_lookup_backend,
    engine_memory,
) -> None:
    ...
    try:
        ...
        if resolved.status not in {
            ...
            stats.skipped += 1
            log_warning(
                "Candidate was not sufficiently resolved and remains skipped."
            )
            return
        ...
        if package_id and existing_lookup_backend is not None:
            ...
            if lookup_response.status == BackendStatus.NOT_FOUND:
                ...
                warning = (
                    "Existing-application lookup failed; candidate will not be treated as new. "
        ...
        if package_id and not existing_lookup_confirmed:
            ...
            stats.review_required += 1
            log_warning(
                "Candidate skipped because existing-application state could not be confirmed safely."
            )
            return
        ...
        if apk_report.selected is None:
            ...
            engine_memory.mark_failure(
                ...
                retryable=True,
                metadata={"stage": "apk", "status": apk_report.status.value},
            )
            return
        ...
        if app_name_requires_review(resolved_name):
            ...
            stats.review_required += 1
            name_warning = (
                "Candidate requires review because the resolved application name is not suitable "
                f"for automatic publication: {safe_text(resolved_name, max_length=120)!r}."
            )
            stats.warnings.append(name_warning)
            log_warning(name_warning)
            return

        content_report = run_live_content_intelligence(
        ...
        log_info(
            ...
            f"evidence: {content_report.evidence_count}; "
            f"populated fields: {content_report.populated_fields}."
        )
        ...
        log_info(
            ...
            f"action={action_value}; kind={decision_result.kind.value}; "
            f"confidence={decision_result.confidence:.3f}; "
            f"review={decision_result.requires_review}; blocked={decision_result.blocked}."
        )
        ...
        if decision_result.blocked:
            ...
            engine_memory.mark_success(
                ...
                action=action_value,
                metadata={"reason": decision_result.reason_text},
            )
        ...
        log_info(
            "AI Review Bridge result: " + ai_review_log_summary(ai_review_result)
        )
        ...
        if ai_review_result.blocks_automatic_publish:
            ...
            stats.skipped += 1
            ai_review_reason = (
                "AI Review Bridge held candidate for manual review: "
                + safe_text(ai_review_result.reason, max_length=300)
            )
        ...
        publication_request = PublicationRequest(
            ...
            payload=ApplicationPayload(
                ...
                version=(
                    ...
                    or resolved_value(MetadataField.VERSION)
                ),
                apk_url=apk_report.selected.url,
                source_url=resolved_value(MetadataField.SOURCE_URL)
                or candidate.source_url,
                repository_url=resolved_value(MetadataField.REPOSITORY_URL)
                or candidate.repository_url,
                license=resolved_value(MetadataField.LICENSE),
                category=resolved_value(MetadataField.CATEGORY),
                short_description=getattr(
                    getattr(content_report, "short_description", None),
                    "value",
                    None,
                )
                or resolved_value(MetadataField.SHORT_DESCRIPTION),
                full_description=getattr(
                    getattr(content_report, "full_description", None),
                    "value",
                    None,
                )
                or resolved_value(MetadataField.FULL_DESCRIPTION),
                icon_url=resolved_value(MetadataField.ICON_URL),
                source=str(candidate.source_type),
            ),
            decision_confidence=decision_result.confidence,
            decision_reason="; ".join(
                safe_text(reason.message, max_length=300)
                for reason in decision_result.reasons
            ),
            run_id=run_id,
            candidate_identity=package_id or candidate.name,
        )
        ...
        log_info(
            ...
            f"external_write={'yes' if external_write_happened else 'no'}."
        )

        if publication_outcome.error:
            log_warning(
                "Publisher outcome: "
                + safe_text(publication_outcome.error, max_length=300)
            )
        ...
        stats.skipped += 1
        error_message = (
            "Processing failed for candidate "
        ...
        try:
            engine_memory.mark_failure(
                ...
                error_message,
                retryable=True,
                metadata={"stage": "candidate-pipeline"},
            )
...
def run_discovery_phase(
    *,
    ...
    try:
        ...
        # ==========================================
        # PARALLEL PROCESSING (CORE FIX)
        # ==========================================
        log_info("Starting parallel processing of candidates.")
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {
                executor.submit(
                    ...
                    publisher_backend_name=publisher_backend_name,
                    publisher_external_writes_enabled=publisher_external_writes_enabled,
                    existing_lookup_backend=existing_lookup_backend,
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    stats.skipped += 1
                    error_message = (
                        "Unexpected parallel processing failure for candidate "
                        f"{safe_text(candidate.name, max_length=120)!r}: "
                        f"{sanitize_exception(exc)}"
                    )
                    stats.warnings.append(error_message)
...
def _phase_report_payload(
    ...
    return {
        ...
        "error": phase.error,
        "duration_seconds": round(phase.duration_seconds, 3),
    }
    ...
    *,
    ...
    return {
        ...
        "content": {
            ...
            "review_required": stats.content_candidates_review_required,
        ...
        "publisher": {
            ...
            "mode": "supabase-live" if config.publishing_enabled else "diagnostic-dry-run",
            "published": stats.published,
            "updated": stats.updated,
            "repaired": stats.repaired,
            ...
            "external_write": stats.published + stats.updated + stats.repaired > 0,
        },
        "discovery": {
            "sources_succeeded": stats.discovery_sources_succeeded,
            "sources_failed": stats.discovery_sources_failed,
            "duplicates_removed": stats.discovery_duplicates_removed,
            "invalid_candidates_removed": stats.discovery_invalid_removed,
        },
...
def print_human_report(
    *,
    ...
    print("Publisher")
    print("  Connected: yes")
    print("  Mode: " + ("supabase live" if config.publishing_enabled else "diagnostic dry-run"))
    print(f"  Published: {stats.published}")
    print(f"  Updated: {stats.updated}")
    print(f"  Repaired: {stats.repaired}")
