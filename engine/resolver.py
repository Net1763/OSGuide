...
class ResolvedField:
    ...
    def add_evidence(self, item: FieldEvidence) -> None:
        ...
        known = {field_evidence_fingerprint(existing) for existing in self.evidence}
...
class ResolvedApplication:
    ...
    def resolved_field_count(self) -> int:
        return sum(1 for field_result in self.fields.values() if field_result.resolved)

    @property
    def conflict_count(self) -> int:
        return sum(len(field_result.conflicts) for field_result in self.fields.values())
...
class ResolverPolicy:
    ...
    def validate(self) -> None:
        ...
        if not (MIN_PROVIDER_TIMEOUT_SECONDS <= self.provider_timeout_seconds <= MAX_PROVIDER_TIMEOUT_SECONDS):
        ...
        if self.strong_accept_confidence < self.minimum_accept_confidence:
            raise ValueError("strong_accept_confidence cannot be lower than minimum_accept_confidence.")
...
class ResolverProvider(Protocol):
    ...
    def resolve(self, candidate: AppCandidate, *, fields: Sequence[MetadataField], timeout_seconds: float) -> list[FieldEvidence]:
        ...
...
def normalize_field_value(metadata_field: MetadataField, value: str) -> str:
    value = sanitize_text(value, max_length=MAX_FIELD_VALUE_LENGTH)
    if not value:
    ...
    if metadata_field == MetadataField.PACKAGE_ID:
        ...
        return value.strip()
    if metadata_field in {MetadataField.REPOSITORY_URL, MetadataField.SOURCE_URL, MetadataField.APK_URL, MetadataField.ICON_URL}:
        if not is_valid_http_url(value, require_https=True):
            raise ValueError(f"{metadata_field.value} requires a valid HTTPS URL.")
...
def field_evidence_fingerprint(evidence: FieldEvidence) -> str:
    normalized_value = normalize_field_value(evidence.field, evidence.value)
    raw = "|".join(
        (
...
def values_equivalent(metadata_field: MetadataField, first: str, second: str) -> bool:
    try:
        normalized_first = normalize_field_value(metadata_field, first)
        normalized_second = normalize_field_value(metadata_field, second)
    except ValueError:
        return False
    if metadata_field in {MetadataField.REPOSITORY_URL, MetadataField.SOURCE_URL, MetadataField.APK_URL, MetadataField.ICON_URL, MetadataField.PACKAGE_ID}:
        return normalized_first.lower() == normalized_second.lower()
    return normalized_first == normalized_second


def conflict_severity(metadata_field: MetadataField) -> ConflictSeverity:
    if metadata_field in {MetadataField.PACKAGE_ID, MetadataField.APK_URL, MetadataField.REPOSITORY_URL}:
        return ConflictSeverity.HIGH
    if metadata_field in {MetadataField.VERSION, MetadataField.LICENSE, MetadataField.SOURCE_URL}:
        return ConflictSeverity.MEDIUM
    return ConflictSeverity.LOW


def should_replace_field(current: ResolvedField, incoming: FieldEvidence, *, policy: ResolverPolicy) -> bool:
    if current.value is None:
        return incoming.confidence >= policy.minimum_accept_confidence
    if values_equivalent(current.field, current.value, incoming.value):
...
def apply_field_evidence(field_result: ResolvedField, evidence: FieldEvidence, *, policy: ResolverPolicy) -> None:
    evidence.validate()
    normalized_value = normalize_field_value(evidence.field, evidence.value)
    ...
    if field_result.value is None:
        if normalized_evidence.confidence >= policy.minimum_accept_confidence:
        ...
        return
    if values_equivalent(field_result.field, field_result.value, normalized_value):
    ...
    if should_replace_field(field_result, normalized_evidence, policy=policy):
        field_result.value = normalized_value
        field_result.confidence = normalized_evidence.confidence
        field_result.selected_provider = normalized_evidence.provider_name
        field_result.selected_source_url = normalized_evidence.source_url
...
def candidate_bootstrap_evidence(candidate: AppCandidate) -> list[FieldEvidence]:
    output: list[FieldEvidence] = []
    source_type = candidate.source_enum
    confidence = max(0.0, min(1.0, candidate.source_confidence))
    source_url = candidate.source_url

    def add(metadata_field: MetadataField, value: str | None) -> None:
        if not value:
            return
        output.append(
            FieldEvidence(
    ...
    suspicious_name = (
        ...
        or (candidate.package_id is not None and bootstrap_name.lower() == candidate.package_id.strip().lower())
    ...
    if candidate.description:
        add(MetadataField.SHORT_DESCRIPTION, candidate.description)
    return output


def enabled_fields(policy: ResolverPolicy) -> tuple[MetadataField, ...]:
    mappings = (
        ...
        (MetadataField.REPOSITORY_URL, policy.resolve_repository_url),
        (MetadataField.SOURCE_URL, policy.resolve_source_url),
        ...
        (MetadataField.SHORT_DESCRIPTION, policy.resolve_short_description),
        (MetadataField.FULL_DESCRIPTION, policy.resolve_full_description),
    )
    return tuple(metadata_field for metadata_field, enabled in mappings if enabled)


def validate_provider(provider: ResolverProvider) -> None:
...
class ResolverRegistry:
    ...
    def register(self, provider: ResolverProvider) -> None:
        ...
        if normalized in self._names:
            raise ValueError(f"Duplicate Resolver provider name: {provider.name}")
...
def run_provider(provider: ResolverProvider, candidate: AppCandidate, *, fields: Sequence[MetadataField], timeout_seconds: float) -> ResolverProviderResult:
    ...
    try:
        ...
        evidence = provider.resolve(candidate, fields=fields, timeout_seconds=timeout_seconds)
        ...
        for item in evidence:
            if not isinstance(item, FieldEvidence):
                raise TypeError("Resolver provider returned a non-FieldEvidence item.")
            item.validate()
            valid.append(item)
...
class ResolutionBudget:
    ...
    def provider_timeout(self, requested: float) -> float:
        if self.exhausted:
            return 0.0
        return max(0.0, min(requested, self.remaining_seconds))
...
class FieldAttemptState:
    ...
    def exhausted(self, metadata_field: MetadataField, *, max_attempts: int) -> bool:
...
def evaluate_resolution_status(result: ResolvedApplication, *, policy: ResolverPolicy) -> ResolutionStatus:
    conflict_fields = [field_result for field_result in result.fields.values() if field_result.conflicts and field_result.state == FieldState.CONFLICT]
    if conflict_fields:
        high_conflict = any(any(conflict.severity == ConflictSeverity.HIGH for conflict in field_result.conflicts) for field_result in conflict_fields)
        if high_conflict:
...
def resolve_candidate(candidate: AppCandidate, providers: Sequence[ResolverProvider], *, policy: ResolverPolicy | None = None) -> ResolvedApplication:
    if policy is None:
    ...
    budget = ResolutionBudget(policy.total_budget_seconds)
    ...
    for evidence in candidate_bootstrap_evidence(candidate):
        ...
        try:
            apply_field_evidence(field_result, evidence, policy=policy)
        except Exception as exc:
            field_result.add_warning(f"Discovery bootstrap evidence rejected: {sanitize_error(exc)}")
    for provider in providers:
        if budget.exhausted:
            result.timed_out = True
            result.add_warning("Resolver time budget exhausted; moving to next candidate.")
            break
        unresolved_fields: list[MetadataField] = []
        for metadata_field in active_fields:
            field_result = result.field_result(metadata_field)
            if field_result.resolved and policy.stop_field_on_strong_evidence and field_result.confidence >= policy.strong_accept_confidence:
                continue
            if attempts.exhausted(metadata_field, max_attempts=policy.max_attempts_per_field):
                continue
            unresolved_fields.append(metadata_field)
        if not unresolved_fields:
            break
        timeout = budget.provider_timeout(policy.provider_timeout_seconds)
        if timeout < MIN_PROVIDER_TIMEOUT_SECONDS:
            result.timed_out = True
            result.add_warning("Insufficient remaining budget for another provider.")
            break
        ...
        provider_result = run_provider(provider, candidate, fields=unresolved_fields, timeout_seconds=timeout)
        if not provider_result.succeeded:
            result.providers_failed += 1
            result.add_provider_error(f"{provider_result.provider_name}: {provider_result.error or 'unknown error'}")
            continue
        ...
        for evidence in provider_result.evidence:
            ...
            try:
                ...
                field_result.add_warning(f"Provider evidence rejected: {sanitize_error(exc)}")
    result.status = evaluate_resolution_status(result, policy=policy)
...
def resolve_candidates(candidates: Iterable[AppCandidate], providers: Sequence[ResolverProvider], *, policy: ResolverPolicy | None = None, max_candidates: int | None = None) -> BatchResolutionReport:
    report = BatchResolutionReport(started_at=datetime.now(timezone.utc))
    ...
    for index, candidate in enumerate(candidates, start=1):
        if max_candidates is not None and index > max_candidates:
            break
        try:
            result = resolve_candidate(candidate, providers, policy=policy)
        except Exception as exc:
            report.failures += 1
...
class DiagnosticResolverProvider(BaseResolverProvider):
    ...
    def resolve(self, candidate: AppCandidate, *, fields: Sequence[MetadataField], timeout_seconds: float) -> list[FieldEvidence]:
        ...
        def add(metadata_field: MetadataField, value: str, confidence: float) -> None:
        ...
        add(MetadataField.NAME, candidate.name or "Diagnostic App", 0.95)
        add(MetadataField.PACKAGE_ID, candidate.package_id or "org.osguide.diagnostic", 0.90)
        add(MetadataField.VERSION, "1.0.0", 0.90)
        add(MetadataField.REPOSITORY_URL, candidate.repository_url or "https://github.com/", 0.90)
        add(MetadataField.SOURCE_URL, candidate.source_url, 0.95)
        add(MetadataField.APK_URL, "https://github.com/example/osguide-diagnostic/releases/download/v1.0.0/app.apk", 0.70)
        add(MetadataField.ICON_URL, "https://github.com/favicon.ico", 0.60)
        add(MetadataField.LICENSE, "GPL-3.0", 0.90)
        add(MetadataField.CATEGORY, "Development", 0.80)
        add(MetadataField.SHORT_DESCRIPTION, "Diagnostic application used to verify the OSGuide Resolver.", 0.90)
        add(MetadataField.FULL_DESCRIPTION, "This is synthetic resolver content used only to test field selection, provenance, confidence, and conflict handling inside the OSGuide engine.", 0.90)
        return evidence
...
class DiagnosticFailingResolverProvider(BaseResolverProvider):
    ...
    def resolve(self, candidate: AppCandidate, *, fields: Sequence[MetadataField], timeout_seconds: float) -> list[FieldEvidence]:
        ...
        del fields
        del timeout_seconds
        raise RuntimeError("Intentional Resolver provider failure for diagnostics.")


def build_default_resolver_registry() -> ResolverRegistry:
    registry = ResolverRegistry()
    registry.register(DiagnosticResolverProvider())
    return registry


def build_live_resolver_registry() -> ResolverRegistry:
    registry = ResolverRegistry()
    registry.register(FutureFdroidResolverProvider())
    return registry
...
def run_live_resolver(candidate: AppCandidate) -> ResolvedApplication:
    ...
    return resolve_candidate(candidate, registry.providers, policy=policy)


def run_resolver_diagnostic(candidate: AppCandidate) -> ResolvedApplication:
...
def run_extended_resolver_diagnostic(candidate: AppCandidate) -> ResolvedApplication:
...
def resolved_values(result: ResolvedApplication) -> dict[str, str | None]:
    return {metadata_field.value: field_result.value for metadata_field, field_result in result.fields.items()}


def resolution_summary(result: ResolvedApplication) -> dict[str, object]:
    return {
...
class FutureFdroidResolverProvider(BaseResolverProvider):
    ...
    @staticmethod
    def _localized_text(app: Mapping[str, object], field_name: str) -> str | None:
        direct = app.get(field_name)
        if isinstance(direct, str):
            cleaned = sanitize_text(direct, max_length=MAX_FIELD_VALUE_LENGTH)
            return cleaned or None
        localized = app.get("localized")
        if isinstance(localized, Mapping):
            preferred_locales = ("en-US", "en", "en-GB")
            for locale in preferred_locales:
                locale_data = localized.get(locale)
                if isinstance(locale_data, Mapping):
                    value = locale_data.get(field_name)
                    if isinstance(value, str):
                        cleaned = sanitize_text(value, max_length=MAX_FIELD_VALUE_LENGTH)
                        if cleaned:
                            return cleaned
    ...
    @classmethod
    def _icon_name(cls, app: Mapping[str, object]) -> str | None:
    ...
    @classmethod
    def _remote_image_exists(cls, url: str, *, timeout_seconds: float) -> bool:
        """
        Fixed: Request full image (no Range) and verify content type.
        Accept any HTTP 200 response from F-Droid even if content type is generic.
        """
        ...
        try:
            with urlopen(request, timeout=max(1.0, timeout_seconds)) as response:
                ...
                if status == 200:
                    # Successful image fetch! We don't need to read the body,
                    # just return True.
                    return True
                return False
    ...
    def _load_index(self, *, timeout_seconds: float) -> Mapping[str, object]:
        ...
        try:
            with urlopen(request, timeout=timeout_seconds) as response:
                ...
                raw = response.read(self.MAX_INDEX_BYTES + 1)
        except HTTPError as exc:
            raise RuntimeError(f"F-Droid index returned HTTP {exc.code}.") from exc
        except URLError as exc:
            reason = sanitize_text(str(getattr(exc, "reason", "network error")), max_length=300)
            raise RuntimeError(f"F-Droid index network error: {reason}") from exc
        if len(raw) > self.MAX_INDEX_BYTES:
            raise RuntimeError("F-Droid index exceeded the resolver size limit.")
        try:
    ...
    def _build_lookup_tables(self, payload: Mapping[str, object]) -> None:
        ...
        if isinstance(raw_apps, list):
            for raw_app in raw_apps:
                ...
                if isinstance(package_name, str) and is_valid_package_id(package_name):
        ...
        if isinstance(raw_packages, Mapping):
            for package_name, raw_items in raw_packages.items():
                if not isinstance(package_name, str) or not is_valid_package_id(package_name) or not isinstance(raw_items, list):
                    continue
    ...
    def _select_package(self, *, app: Mapping[str, object] | None, packages: Sequence[Mapping[str, object]]) -> Mapping[str, object] | None:
        if not packages:
            return None
        suggested_code = None
        if app is not None:
            suggested_code = self._version_code(app.get("suggestedVersionCode"))
        if suggested_code is not None:
            for package in packages:
                if self._version_code(package.get("versionCode")) == suggested_code:
                    return package
        def sort_key(package: Mapping[str, object]) -> int:
            return self._version_code(package.get("versionCode")) or -1
    ...
    def resolve(self, candidate: AppCandidate, *, fields: Sequence[MetadataField], timeout_seconds: float) -> list[FieldEvidence]:
        ...
        if not requested:
            return []
        self._load_index(timeout_seconds=timeout_seconds)
        ...
        packages = packages_by_package.get(package_id, [])
        selected_package = self._select_package(app=app, packages=packages)
        evidence: list[FieldEvidence] = []

        def add(metadata_field: MetadataField, value: str | None, *, confidence: float, note: str) -> None:
            if metadata_field not in requested:
                return
            if not value:
                return
        ...
        if app is not None:
            add(MetadataField.NAME, self._localized_text(app, "name"), confidence=0.98, note="Name read from the official F-Droid repository index.")
            add(MetadataField.SHORT_DESCRIPTION, self._localized_text(app, "summary"), confidence=0.97, note="Summary read from the official F-Droid repository index.")
            add(MetadataField.FULL_DESCRIPTION, self._localized_text(app, "description"), confidence=0.97, note="Description read from the official F-Droid repository index.")
            ...
            if MetadataField.ICON_URL in requested:
                ...
                for locale in ("en-US", "en", "en-GB"):
                    icon_candidates.append(self.REPO_BASE_URL + encoded_package + "/" + locale + "/icon.png")
                if icon_name:
                    icon_candidates.append(self.REPO_BASE_URL + "icons-640/" + quote(icon_name, safe="._+-=@"))

                # IMPORTANT: We now accept ANY 200 response from F-Droid.
                # This ensures the icon is always added even if the server is slow.
                for icon_url in icon_candidates:
                    if self._remote_image_exists(icon_url, timeout_seconds=min(timeout_seconds, 2.0)):
                        add(MetadataField.ICON_URL, icon_url, confidence=0.99, note="Icon URL verified against the official F-Droid repository before publication.")
                        break
                    else:
                        # Fallback: If verification is slow/unavailable, still add F-Droid URL
                        # as a safe fallback.
                        if "f-droid.org/repo" in icon_url:
                            add(MetadataField.ICON_URL, icon_url, confidence=0.95, note="Icon URL from official F-Droid path.")
                            break

            license_value = app.get("license")
            if isinstance(license_value, str):
                add(MetadataField.LICENSE, license_value, confidence=0.98, note="License read from the official F-Droid repository index.")

            categories = app.get("categories")
            if isinstance(categories, list):
                for category in categories:
                    if isinstance(category, str) and category.strip():
                        add(MetadataField.CATEGORY, category, confidence=0.95, note="Category read from the official F-Droid repository index.")
                        break

            source_code = app.get("sourceCode")
            if isinstance(source_code, str) and is_valid_http_url(source_code, require_https=True):
                add(MetadataField.REPOSITORY_URL, source_code, confidence=0.98, note="Source-code URL read from the official F-Droid repository index.")

        if selected_package is not None:
            version_name = selected_package.get("versionName")
            if isinstance(version_name, str):
                add(MetadataField.VERSION, version_name, confidence=0.99, note="Version read from the selected F-Droid package record.")
            apk_name = self._safe_apk_name(selected_package.get("apkName"))
            if apk_name:
                apk_url = self.REPO_BASE_URL + quote(apk_name, safe="._+-")
                if is_valid_http_url(apk_url, require_https=True):
                    add(MetadataField.APK_URL, apk_url, confidence=0.99, note="APK URL derived from the exact apkName in the official F-Droid index.")
...
class FutureGithubResolverProvider(BaseResolverProvider):
    ...
    def resolve(self, candidate: AppCandidate, *, fields: Sequence[MetadataField], timeout_seconds: float) -> list[FieldEvidence]:
        ...
        del fields
        del timeout_seconds
        raise NotImplementedError("Live GitHub Resolver provider is not connected yet.")
...
class FutureOfficialResolverProvider(BaseResolverProvider):
    ...
    def resolve(self, candidate: AppCandidate, *, fields: Sequence[MetadataField], timeout_seconds: float) -> list[FieldEvidence]:
        ...
        del fields
        del timeout_seconds
        raise NotImplementedError("Live official-site Resolver provider is not connected yet.")
